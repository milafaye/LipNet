...
PREDICT_DICTIONARY  = os.path.join(CURRENT_PATH,'..','..','common','dictionaries', 'buthowcanwecontrolevaporation.txt')#'grid.txt')
...
def train(run_name, start_epoch, stop_epoch, img_c, img_w, img_h, frames_n, absolute_max_string_len, minibatch_size):
    curriculum = None #Curriculum(curriculum_rules)
    ...
    # define callbacks
    num_statistic_samples = minibatch_size
    statistics  = Statistics(lipnet, lip_gen.next_val(), decoder, num_statistic_samples, output_dir=os.path.join(OUTPUT_DIR, run_name))
    ...
    print (
        "**********training: training_steps:=", lip_gen.default_training_steps, 
        "validation_steps=", max(1, lip_gen.default_validation_steps),
    ...
    #                     validation_data=lip_gen.next_val(), validation_steps=max(1, lip_gen.default_validation_steps),
...
if __name__ == '__main__':
    ...
    train(
        ...
        absolute_max_string_len=absolute_max_string_len, 
        minibatch_size=minibatch_size)
    # train(run_name, 0, 5000, 3, 100, 50, 75, absolute_max_string_len, minibatch_size)